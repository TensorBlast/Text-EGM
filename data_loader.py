...
class EGMClassificationDataset(Dataset):
    def __init__(self, data_dict, tokenizer=None, args=None):
        self.data = list(data_dict.values())
        self.keys = list(data_dict.keys())
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        if self.args.TA:
            ...
            if augmentation_scheme == 2:
                all_tokens = [cls_id] + quantized_signal_ids + sampled_quantized_augsignal_ids + [sep_id]
            else:
                all_tokens = [cls_id] + quantized_signal_ids + [pad_id] * int(0.25 * len(quantized_signal_ids)) + [sep_id]
        else:
            all_tokens = [cls_id] + quantized_signal_ids + [sep_id]

        # Create attention mask
        attention_mask = np.ones_like(all_tokens)
        
        # For classification, we don't need to mask tokens - the model will predict the label
        return torch.LongTensor(all_tokens), torch.tensor(afib_label, dtype=torch.long), \
               torch.tensor(attention_mask, dtype=torch.int), key, \
               torch.tensor(min_val, dtype=torch.float32), torch.tensor(max_val, dtype=torch.float32)
